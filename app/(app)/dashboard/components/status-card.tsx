...
export function StatusCard({
  ...
  const cardClassName = cn(
    "group relative block overflow-hidden rounded-3xl sm:rounded-[2.5rem] border p-6 sm:p-8 shadow-xs transition-all hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.01] animate-in fade-in slide-in-from-bottom-4 duration-700",
    redirectUrl || onClick ? "cursor-pointer" : "cursor-default",
    positive
  ...
  if (redirectUrl) {
    return (
      <Link href={redirectUrl as Route} className={cardClassName} onClick={onClick}>
        {cardContent}
      </Link>
    );
  }
