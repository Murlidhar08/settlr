...
export default function PartiesScreen() {
    return (
        <div className="min-h-screen bg-background text-foreground pb-28">
            {/* Header */}
            <header className="sticky top-0 z-20 bg-background/90 backdrop-blur supports-backdrop-filter:bg-background/70">
            ...
            <nav className="fixed bottom-0 left-0 right-0 border-t bg-background/90 backdrop-blur">
                <ul className="flex justify-around px-6 py-3">
                    ...
                    <NavItem icon={<Users />} label="Parties" active />
...
function NavItem({ icon, label, active }: any) {
    return (
        <li className={`flex flex-col items-center gap-1 text-xs ${active ? "text-primary" : "text-muted-foreground"}`}>
            {icon}
